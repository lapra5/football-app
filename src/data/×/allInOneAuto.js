...
const fetchTeamsFromFootballData = async (leagueId) => {
...
const fetchJapanesePlayers = async (teamId) => {
  ...
  return json.response.map((player) => player.player.name);
};

const fetchLogoFromAPIFootball = async (englishName) => {
  const res = await fetch(
...
const fetchLogoFromTheSportsDB = async (englishName) => {
  const res = await fetch(
...
const fetchJapaneseNameFromWikidata = async (englishName) => {
  try {
    const res = await fetch(
